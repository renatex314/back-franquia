...
const getFranquiaCursoByFields = async (props: GetByFieldsProps<FranquiaCurso>) => {
  const connection = getConnection();

  try {
    const franquiaCurso: FranquiaCurso = await connection.select().from('franquia_curso').where(props).first();
